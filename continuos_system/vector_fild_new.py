...
class State(Triangle):
	...
	def vector_field_point(self, point, patch_num, state_num, v_max):
		...
		# print self.patch_list[patch_num - 1]
		# print 'i', [self.v1v2v3(state_num)[2]]
		# print 'g1 g2 g3=', g1_angle, g2_angle, g3_angle
...
class Robot(State):
	...
	def move(self, current_pos, current_vel, current_state = 1, current_patch = 1, dt = 0.05):
		if self.reinitial == True:
			print('agent', self.agent_id, 'current vel', self.curent_velocity, 'reinitialized')
	...
	def collision_check(self, p1, p2, v1, v2, agent_id, teammate_id):  # Velocity Obstacle method
		...
		# #print 'v2_1', v2_1, p_vo_right, p_vo_left, 'pvo right left '
		...
		polygon_vo = tuple([round((p1[0] + v2_1[0] - world_xmin) * world_scale), round((p1[1] + v2_1[1] - world_ymin) * world_scale), round((p1[0] + p_vo_right[0] - world_xmin) *
		                                                                                                                                    world_scale), \
...
def collision_avoidance(agent_1,agent_2):
	...
	def collision_free_velocity(r1,r2):
		...
		# 		if not is_point_in_tri(g_set_ordered[i], [v2_1, p_vo_right, p_vo_left]): # if vertex of the g_polyhedral is outside of the RVO trinagle it is collision-free
		...
		# 	v_optimal = v1
		#
		# return v_optimal, v_sides, is_collision, [g_set_ordered, v2_1, p_vo_right, p_vo_left]
	
	# def decide_velocity(v1_opt, v2_opt, v1_dir, v2_dir):
	# 	print('v1_dir=', v1_dir, 'v2_dir=', v2_dir)
	# 	print('v1_opt=', v1_opt, 'v2_opt=', v2_opt)
	# 	if v1_dir == [False, False]:
	# 		agent_1.curent_velocity = [0.,0.]
	...
	# 				agent_2.curent_velocity = [0., 0.]
	# 		elif v2_dir[0] == True and v2_opt[0][1] * v2_opt[0][0] > 0.1:
	# 			agent_2.curent_velocity = v2_opt[0]
	# 		elif v2_opt[1][1] * v2_opt[1][0] > 0.1:
	# 			agent_2.curent_velocity = v2_opt[1]
	# 	elif v1_dir[0] == True:
	# 		if v2_dir[0] == True:
	# 			print('Avoiding collision side: Right')
	# 			agent_1.curent_velocity = v1_opt[0]
	# 			agent_2.curent_velocity = v2_opt[0]
	# 		elif v2_dir[1] == False:
	# 			agent_1.curent_velocity = v1_opt[0]
	# 			print 'right avoidance'
	# 	elif v1_dir[1] == True:
	# 		if v2_dir[1] == True:
	# 			agent_1.curent_velocity = v1_opt[1]
	# 			agent_2.curent_velocity = v2_opt[1]
	# 			print('Avoiding collision side: Left')
	# 		elif v2_dir[0] == False:
	# 			agent_1.curent_velocity = v1_opt[1]
	...
	if is_colliding:
		...
		for i in range(2):
			...
			polygon_vo = tuple([round((p1[0] + v2_1[0] - world_xmin) * world_scale), round((p1[1] + v2_1[1] - world_ymin) * world_scale), round((p1[0] + p_vo_right[0] - world_xmin) *
			                                                                                                                                    world_scale), \
			                    round((p1[1] + p_vo_right[1] - world_ymin) * world_scale), round((p1[0] + p_vo_left[0] - world_xmin) * world_scale),
			                    round((p1[1] + p_vo_left[1] - world_ymin) * world_scale)])
			x1_v = round((ro1.current_position[0] - world_xmin) * world_scale)
			y1_v = round((ro1.current_position[1] - world_ymin) * world_scale)
			x2_v = round((ro1.current_position[0] - world_xmin + v_optimal[0]) * world_scale)
